function [positions, time] = my_KCF_patch_tracker(now_frame,prev_frame, pos, target_sz)
...
features.gray = false;
features.hog = false;
padding = 0;  %extra area surrounding the target
...
% 	positions = zeros(numel(now_frame), 2);  %to calculate precision
    positions = zeros(2, 2);

	for frame = 1:2,   % frame = 1:numel(img_filses),
		%load image
        if frame==1
            im=prev_frame;
        else
		...
		if frame > 1,
            ...
            %新的pose在此就已经产生了,但window size其实是没有变化的
		end
%下面的代码，是在为接下来的循环做准备，他会储存上一帧的结果，并update.所以我可以从此处下手
