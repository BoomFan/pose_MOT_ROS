...
while ~isDone(obj.reader)
    ...
    % next four are for debug use
    tracked_bbox = [];
    tracked_labels = {};
    detected_bbox = bboxes;
    detected_labels = {};
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects()
        ...
        obj.reader = vision.VideoFileReader('bolt.mp4');
        obj.writer = vision.VideoFileWriter('bolt_rcnn_KCF_new_strategy.avi');
...
% current frame, and update its bounding box accordingly.
    ...
    function predictNewLocationsOfTracks()
        for i = 1:length(tracks)
            bbox = tracks(i).bbox;
            [positions, time] = my_KCF_tracker(frame,tracks(i).prev_frame, [(bbox(1,2)+bbox(1,4)/2),(bbox(1,1)+bbox(1,3)/2)], [bbox(1,4),bbox(1,3)]);
%             [positions, time] = my_KCF_patch_tracker(frame,prev_frame, [(bbox(1,2)+bbox(1,4)/2),(bbox(1,1)+bbox(1,3)/2)], [bbox(1,4),bbox(1,3)])
%             [positions, time] = my_KCF_tracker(frame,prev_frame, (bbox(1,1:2)+bbox(1,3:4)),[bbox(1,4),bbox(1,3)]);
            % Predict the current location of the track.
%             predictedCentroid = predict(tracks(i).kalmanFilter);
            predictedCentroid = [positions(2,2),positions(2,1)];
            % Shift the bounding box so that its center is at 
            % the predicted location.
            predictedCorner = int32(predictedCentroid) - bbox(3:4) / 2;
            tracks(i).bbox = [predictedCorner, bbox(3:4)];
...
% indices of tracks and detections that remained unassigned. 
    ...
    function [assignments, unassignedTracks, unassignedDetections] = ...
        ...
        overlap_ratio = cost;
        for i = 1:nTracks
            now_centroid_x = double(tracks(i).bbox(1)+tracks(i).bbox(3));
            now_centroid_y = double(tracks(i).bbox(2)+tracks(i).bbox(4));
            for j=1:nDetections
                % Define two bounding boxes in the format [x y width height].
%                 cost(i, j) = sqrt((now_centroid_x-centroids(j,1))^2+(now_centroid_y-centroids(j,2))^2);
                overlap_ratio(i, j) = bboxOverlapRatio(tracks(i).bbox,bboxes(j,:));
                cost(i, j) = 1 - overlap_ratio(i, j);
                
                % for debug use
                detected_labels{j} = strcat('           rcnn ', num2str(j));
                
            end
            
            % for debug use
            tracked_bbox(i,:) = tracks(i).bbox;
            tracked_labels{i} = strcat('     kcf ', num2str(i));
        end
...
% visible count by 1. Finally, the function sets the invisible count to 0. 
    ...
    function updateAssignedTracks()
        ...
        for i = 1:numAssignedTracks
            ...
            tracks(trackIdx).prev_frame = frame;
        end
    end
...
%                     bboxes, labels,'Color',{'red'});
                frame = insertShape(frame, ...
                                            ...
                                            'Color', 'red');
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                    tracked_bbox, tracked_labels,'Color',{'cyan'},'TextBoxOpacity',0.2);
                frame = insertObjectAnnotation(frame, 'rectangle', ...
                    detected_bbox, detected_labels,'TextBoxOpacity',0.2);
