...
function PedestrianTrackingMovingCamera_RCNN_KCF()
...
obj.writer = vision.VideoFileWriter('boy_rcnn_KCF_matlab_moving_camera.avi');
...
while cont
    ...
    centroids=centroids_rcnn{rcnncont};
    bboxes=double(bboxes_rcnn{rcnncont});
    scores = scores_rcnn{rcnncont}*100;
...
% Y-coordinate of the pedestrian's feet.
    ...
    function obj = setupSystemObjects(videoFile,scaleDataFile)
        ...
        obj.videoPlayer = vision.VideoPlayer('Position', [20, 20, 1880, 920]);
...
% prediction of the size.
    ...
    function predictNewLocationsOfTracks()
        for i = 1:length(tracks)
            ...
            % Predict the current location of the track using KCF.
            [positions, time] = my_KCF_tracker(frame,tracks(i).prev_frame,...
                [(bbox(1,2)+bbox(1,4)/2),(bbox(1,1)+bbox(1,3)/2)], [bbox(1,4),bbox(1,3)]);
...
%     end
    ...
    function [assignments, unassignedTracks, unassignedDetections] = ...
        ...
        overlap_ratio = cost;
        for i = 1:nTracks
%             now_centroid_x = double(tracks(i).bboxes(end,1)+tracks(i).bboxes(end,3));
%             now_centroid_y = double(tracks(i).bboxes(end,2)+tracks(i).bboxes(end,4));
            for j=1:nDetections
                ...
                overlap_ratio(i, j) = bboxOverlapRatio(tracks(i).predPosition,bboxes(j,:));
            ...
            % for debug use
            tracked_bbox(i,:) = tracks(i).predPosition;
